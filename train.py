...
from args import *
from fcv_dataloder import FVCDataset
...
normalize = transforms.Normalize(mean=[0.22, 0.37, 0.73],std=[1.61, 1.75, 1.80])
...
print("***********- loading model -*************")
model =MobileNetV2(num_classes=data_config.num_class).cuda()
...
class trainer:
    ...
    def train_epoch(self, loader,epoch):
        ...
        for batch_idx, (imgs, labels) in enumerate(tqdm_loader):
            ...
            imgs, labels=imgs.permute(0,3,1,2).cuda(), labels.cuda()
    ...
    def valid_epoch(self, loader, epoch):
        ...
        for batch_idx, (imgs, labels) in enumerate(loader):
            with torch.no_grad():
                batch_imgs = imgs.permute(0,3,1,2).cuda()
