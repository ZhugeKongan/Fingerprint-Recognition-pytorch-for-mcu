...
def get_fp_region(img_path, crop_width, crop_height):
    ...
    #图像自动阈值分割,转换成二值图像，计算阈值
    thresh = filters.threshold_otsu(image)
    #闭运算（closing)，将不连续的区域连起来
...
class Prepare_Data:
    ...
    def prepare_train_data(self, save_url='', save_img=False):
        ...
        ExpWidth = EXPAND_WIDTH
        ExpHeight = EXPAND_HEIGHT
        finger_idx = 0#指纹类别对应的finger_idx指纹
        finger_id = 0#当前指纹类别
        for item in img_list:
            ...
            finger_id_tmp = parse_file_name2(item)#指纹类别
            if (int(finger_id_tmp) != finger_id):
                finger_id = int(finger_id_tmp)
                finger_idx = 0
            finger_id=finger_id%100-1
            finger_ids=np.zeros(10)
            finger_ids[finger_id]=1
            ...
            img = img.crop([crop_l, crop_t, crop_r, crop_b])#左上右下
            ...
            train_labels.append(finger_ids)
            if save_img == True:
...
if __name__ == '__main__':
    data_path = "./dataset_FVC2000_DB4_B/DB4_B/"
    out_path = "./dataset_FVC2000_DB4_B/dataset/"
    # global variables
    image_width = 64
    image_height = 64

    train_data_pre = Prepare_Data(image_width, image_height, data_path)
